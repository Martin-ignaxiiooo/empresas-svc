# Empresas y Usuarios — Microservicio de riesgo empresas

Microservicio correspondiente al **caso caso01 — WorkMatch** (Portal de busqueda de empleo (empresas)) de la Evaluación Parcial N°1.
...
administra los datos y la lógica del dominio de Empresas y Usuarios del caso caso01 (WorkMatch). Su base de datos es una **H2 en memoria** (un solo microservicio por base), cumpliendo aislamiento de datos por dominio.
...
|--------|------|-------------|
| GET | `/api/empresas` | Lista todos los recursos |
| GET | `/api/empresas/{id}` | Obtiene un recurso por id |
| POST | `/api/empresas` | Crea un recurso |
| PUT | `/api/empresas/{id}` | Actualiza un recurso |
| DELETE | `/api/empresas/{id}` | Elimina un recurso |
