# Empresas y Usuarios — Arquitectura del microservicio

## Componentes

| Componente | Rol |
|---|---|
| `EmpresaApplication` | Punto de entrada Spring Boot |
| `controller/EmpresaController.java` | API REST expuesta en `/api/empresas` |
| `service/EmpresaService.java` | Lógica de negocio y transacciones |
| `repository/EmpresaRepository.java` | Acceso JPA a H2 |
| `model/Empresa.java` | Entidad persistida en tablas H2 |
...
`Cliente/Sistema → API Gateway → EmpresaController → EmpresaService → EmpresaRepository → H2`
