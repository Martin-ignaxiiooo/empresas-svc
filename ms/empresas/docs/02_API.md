# Empresas y Usuarios — Contrato de la API REST

## Base

- **Base path**: `/api/empresas`
...
|--------|------|-------------------|-------------|
| GET | `/api/empresas` | 200 | Lista todos los recursos |
| GET | `/api/empresas/{id}` | 200 / 404 | Obtiene un recurso por id |
| POST | `/api/empresas` | 201 / 400 | Crea un recurso |
| PUT | `/api/empresas/{id}` | 200 / 404 / 400 | Actualiza un recurso |
| DELETE | `/api/empresas/{id}` | 204 / 404 | Elimina un recurso |
...
| industria | String | No | Campo del dominio |
